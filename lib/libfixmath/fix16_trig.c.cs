...
namespace libfixmath
{
    ...
    {
        const int CHAR_BIT = sizeof(char);

#if FIXMATH_SIN_LUT
#elif !FIXMATH_NO_CACHE
        static fix16_t[] _fix16_sin_cache_index = new fix16_t[4096];
        static fix16_t[] _fix16_sin_cache_value= new fix16_t[4096];
#endif

#if !FIXMATH_NO_CACHE
        static fix16_t[,] _fix16_atan_cache_index = new fix16_t[2, 4096];
        static fix16_t[] _fix16_atan_cache_value = new fix16_t[4096];
#endif


        public static fix16_t fix16_sin_parabola(fix16_t inAngle)
        {
        ...
        public static fix16_t fix16_sin(fix16_t inAngle)
        {
...
#if !FIXMATH_FAST_SIN // Most accurate version, accurate to ~2.1%
        ...
        public static fix16_t fix16_cos(fix16_t inAngle)
        {
            return fix16_sin(inAngle + (fix16_pi >> 1));
        }

        public static fix16_t fix16_tan(fix16_t inAngle)
        {
            return fix16_sdiv(fix16_sin(inAngle), fix16_cos(inAngle));
        }

        public static fix16_t fix16_asin(fix16_t x)
        {
            if ((x > fix16_one)
                || (x < -fix16_one))
                return 0;
            ...
            fix16_t ret;
            ret = (fix16_one - fix16_mul(x, x));
            ret = fix16_div(x, fix16_sqrt(ret));
            ret = fix16_atan(ret);
            return ret;
        }

        public static fix16_t fix16_acos(fix16_t x)
        {
            return ((fix16_pi >> 1) - fix16_asin(x));
        }

        public static fix16_t fix16_atan2(fix16_t inY, fix16_t inX)
        {
            fix16_t abs_inY, mask, angle, r, r_3;

#if !FIXMATH_NO_CACHE
            uintptr_t hash = (uintptr_t)(inX ^ inY);
            hash ^= hash >> 20;
            hash &= 0x0FFF;
        ...
        public static fix16_t fix16_atan(fix16_t x)
        {
